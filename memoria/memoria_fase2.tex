...
\begin{tcolorbox}[colback=SoftGreen,colframe=MainGreen,boxrule=0.8pt,arc=2pt,
  ...
  \begin{tabular}{ll}
    ...
    \textbf{Multiplexación:} &
      Registro \texttt{mux\_reg} one-hot activo bajo; rota en cada
      \texttt{tic\_1ms}.\\
...
El registro \texttt{mux\_reg} es un one-hot activo bajo que rota hacia la
izquierda en cada \texttt{tic\_1ms}, activando un display diferente en cada
ciclo de refresco:
...
\subsection{Cálculo de los 8 dígitos a mostrar}

Los 8 dígitos se calculan combinacionalmente como señales concurrentes
\texttt{sig\_d1} \ldots \texttt{sig\_d8}, sin variables ni proceso. Cada
señal usa una cadena \texttt{when/else} que considera el modo
\texttt{pres} y aplica la supresión de ceros a la izquierda:

\begin{lstlisting}[caption={Supresión de ceros en \texttt{displays.vhd} (fragmento)}]
-- D2 (decenas): blanco si tanto centenas como decenas son cero
...
-- D3 (centenas o signo negativo si centenas = 0)
...
La función \texttt{decode\_seg} implementa la tabla de conversión. Se ha
eliminado la variable interna que tenía la versión original; ahora usa
\texttt{return} directo en cada rama del \texttt{case}:

\begin{lstlisting}[caption={Función \texttt{decode\_seg} sin variables en \texttt{displays.vhd}}]
function decode_seg(d : std_logic_vector(3 downto 0))
         return std_logic_vector is
begin
    case d is
        when x"0" => return "01111110";  -- 0: a,b,c,d,e,f
        when x"1" => return "00110000";  -- 1: b,c
        when x"2" => return "01101101";  -- 2: a,b,d,e,g
        when x"3" => return "01111001";  -- 3: a,b,c,d,g
        when x"4" => return "00110011";  -- 4: b,c,f,g
        when x"5" => return "01011011";  -- 5: a,c,d,f,g
        when x"6" => return "01011111";  -- 6: a,c,d,e,f,g
        when x"7" => return "01110000";  -- 7: a,b,c
        when x"8" => return "01111111";  -- 8: todos
        when x"9" => return "01111011";  -- 9: a,b,c,d,f,g
        when x"F" => return "00000001";  -- '-': solo g (signo negativo)
...
\begin{table}[H]
  ...
  \begin{tabular}{clcc}
    ...
    \texttt{x"A"} & \texttt{A}         & \texttt{00110111} & Dígito hex A \\
    \texttt{x"B"} & \texttt{P}         & \texttt{00011111} & Reservado \\
