...
\begin{titlepage}
	...
	\begin{flushleft}
		{\fontsize{9}{12}\selectfont\color{white!80} GRADO EN INGENIERÍA DE SISTEMAS ELECTRÓNICOS}\\[1ex]
	...
	\begin{center}
		{\fontsize{8}{10}\selectfont\color{black!40}
			Grado en Ingeniería de Sistemas Electrónicos \quad$\cdot$\quad Diseño Digital 2}
	\end{center}
	\vspace{0.5cm}
...
\end{titlepage}
	...
	\subsection{Algoritmo: Double Dabble}
	
	Hemos implementado el algoritmo \textit{Double Dabble} (también llamado ``shift-and-add-3''), que procesa el número binario bit a bit, de más significativo a menos, en exactamente $N_{\text{BITS}} = 20$ ciclos de reloj. Cada ciclo hace lo siguiente:
	
	\begin{enumerate}[leftmargin=1.8cm, label=\textbf{Paso \arabic*.}]
		\item \textbf{Add-3}: para cada nibble BCD del acumulador, si su valor es
		$\geq 5$, se le suma 3. Esto garantiza que tras el desplazamiento
		siguiente el nibble no desborde el dígito decimal.
		\vspace{2pt}\\
		\textit{Justificación:} si un nibble vale $n \in [5, 9]$, tras un
		shift izquierda valdría $2n \in [10, 18]$, desbordando un dígito decimal.
		Sumando 3 antes ($n + 3 \in [8, 12]$), el shift produce $2(n+3) \in [16, 24]$,
		cuyo carry transporta correctamente $+1$ al nibble superior.
		
		\item \textbf{Shift izquierda}: se desplaza un bit a la izquierda el registro
		combinado $[\texttt{acc\_bcd} \mid \texttt{bin\_reg}]$, introduciendo
		el MSB actual de \texttt{bin\_reg} por la derecha de \texttt{acc\_bcd}.
	\end{enumerate}
	
	\begin{lstlisting}[caption={Núcleo del algoritmo Double Dabble en \texttt{bin\_to\_bcd.vhd}}]
		-- Paso 1: Add-3 a cada nibble si su valor es >= 5
		if acc_bcd(3  downto 0) >= "0101" then n0 := acc_bcd(3  downto 0) + "0011";
		else n0 := acc_bcd(3  downto 0); end if;
		if acc_bcd(7  downto 4) >= "0101" then n1 := acc_bcd(7  downto 4) + "0011";
		else n1 := acc_bcd(7  downto 4); end if;
		-- (idem para n2..n5: centenas, miles, decenas de miles, centenas de miles)
		
		-- Paso 2: Reconstruir vector ajustado y desplazar
		bcd_adj := n5 & n4 & n3 & n2 & n1 & n0;
		
		acc_bcd <= bcd_adj(22 downto 0) & bin_reg(N_BITS-1); -- shift + MSB entrante
	...
	\begin{center}
		\begin{tikzpicture}[
			...
			{\texttt{bits\_left} $>$ 0\\[-1pt]paso Double Dabble}
			(calc);
	...
	\begin{table}[H]
		...
		\begin{tabular}{lll}
			...
			\texttt{CALC}
			& Ejecuta un paso Double Dabble por ciclo
	...
	\begin{itemize}[leftmargin=1.6cm, itemsep=4pt]
		...
		\item \textbf{\texttt{bin\_to\_bcd}}: secuencial con FSM de tres estados (\texttt{IDLE} / \texttt{CALC} / \texttt{FINISH}). El algoritmo \textit{Double Dabble} tarda exactamente 20 ciclos de reloj, lo que es más que suficiente para los valores que puede generar la calculadora ($999 \times 999 = 998\,001$ como máximo).
	\end{itemize}
	...
	En el planteamiento inicial del conversor binario a BCD no se disponía de ningún método conocido para realizar dicha conversión sin emplear divisiones o módulos, operaciones que no son directamente sintetizables con \texttt{std\_logic\_unsigned} y que resultan costosas en área hardware.

	Tras investigar alternativas, se localizó el algoritmo \textit{Double Dabble} (también denominado \textit{shift-and-add-3}), cuya existencia era completamente desconocida para el grupo antes de esta fase. El algoritmo opera exclusivamente con desplazamientos de bits y sumas de la constante~3, dos operaciones triviales en hardware digital.

	\begin{tcolorbox}[colback=SoftOrange, colframe=MainOrange, boxrule=0.8pt, arc=2pt,
		left=0.5cm, right=0.5cm, top=0.3cm, bottom=0.3cm]
		\small\faLightbulb\enspace\textbf{Intuición del algoritmo:}
		El truco de sumar~3 cuando un nibble supera el valor~4 se basa en la diferencia entre la base~16 y la base~10: $16 - 10 = 6$, y la mitad de ese valor es~3. Al sumar~3 antes del desplazamiento se ``pre-compensa'' el desbordamiento decimal que ocurriría tras duplicar el nibble, de modo que el carry se propaga correctamente hacia el dígito BCD superior.
	\end{tcolorbox}

	La comprensión del algoritmo requirió revisar su demostración formal bit a bit para todos los casos posibles de un nibble ($0$--$9$), verificando que la condición add-3 garantiza que ningún nibble supera el valor~9 en ningún momento del proceso. Una vez comprendido, su implementación en VHDL resultó directa.
	...
	\subsection{Gestión de flancos de reloj en VHDL-93 (ModelSim 5.x)}

	Durante la escritura de los testbenches se utilizó inicialmente la función \texttt{rising\_edge(clk)}, disponible en VHDL-93 y posterior. Sin embargo, la versión de ModelSim del laboratorio presenta comportamientos inconsistentes con dicha función en procesos de estímulo, disparando la condición incluso desde estados indefinidos (\texttt{'U'}, \texttt{'X'}).

	Tras investigar la especificación original de VHDL-87, se comprobó que la forma canónica y más robusta para este entorno es:

	\begin{center}
		\texttt{wait until clk'event and clk = '1';}
	\end{center}

	Esta construcción solo dispara la espera cuando el evento corresponde a una transición efectiva hacia \texttt{'1'}, ignorando transiciones desde valores no definidos, lo que resulta más adecuado para la versión de simulador disponible.

	\subsection{Terminación controlada de simulaciones}

	Inicialmente los procesos de estímulo finalizaban con un \texttt{wait} indefinido, lo que dejaba la simulación activa sin producir más eventos. Esto impedía que ModelSim detectase el fin de la simulación y requería detenerla manualmente.

	Se investigó el mecanismo estándar de VHDL para forzar la terminación, encontrando que la forma correcta es:

	\begin{center}
		\texttt{assert false report "Fin de simulacion" severity failure;}
	\end{center}

	Al elevar la severidad a \texttt{failure}, el simulador interrumpe la ejecución de forma ordenada, imprime el mensaje en la consola y libera todos los recursos, sin necesidad de intervención manual.
