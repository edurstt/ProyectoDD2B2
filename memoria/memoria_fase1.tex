...
\end{titlepage}
	...
	\begin{tcolorbox}[colback=SoftBlue, colframe=MainBlue, boxrule=0.8pt, arc=2pt,
		...
		\small\faInfoCircle\enspace\textbf{Restricción de librería:} Siguiendo las
		directrices de la asignatura, todos los módulos emplean
	...
	\begin{lstlisting}[caption={Procedimiento auxiliar \texttt{run\_test} del testbench secuencial}]
		...
		begin
		wait until rising_edge(clk);  -- alinear con el reloj antes de asignar
		bin   <= val;
		start <= '1';
		wait until rising_edge(clk);  -- IDLE detecta start; done pasa a '0'
		start <= '0';
